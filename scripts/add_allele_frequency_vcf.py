...
def make_header(invcf):
    ...
    invcf.header.formats.add(
        "SUM_ALT", "1", "Integer", "Sum of observations from all alternate alleles"
    )
    invcf.header.formats.add(
        "SUM_ALT_AF", "1", "Float", "Sum of alternate allele frequencies"
    )
    if "AD" not in invcf.header.formats.keys():
        invcf.header.formats.add(
            "AD", "2", "Integer", "Number of observation of REF and ALT alleles"
        )
...
parser.add_argument(
    ...
    "-f",
    default="freebayes",
    choices=["freebayes", "delly"],
    help="Format (source) of VCF input [%(default)s]",
)
